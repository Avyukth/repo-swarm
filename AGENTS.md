...
1. **Same repo**
   ...
   - Optional:
     ...
     - Check hooks with `am guard status` and identity with `am mail status`.
...
- Create/ensure product: `am products ensure MyProduct --name "My Product"`.
- Link repo: `am products link MyProduct .`.
- Inspect: `am products status MyProduct`.
- Search: `am products search MyProduct "bd-123 OR \"release plan\"" --limit 50`.
- Product inbox: `am products inbox MyProduct YourAgent --limit 50 --urgent-only --include-bodies`.
- Summaries: `am products summarize-thread MyProduct "bd-123" --per-thread-limit 100 --no-llm`.
